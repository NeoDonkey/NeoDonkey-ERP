# Decision Record: Chart of Accounts Template Onboarding and Opening Balance (Eröffnungsbilanz) Initialization Engine
...
How should NeoDonkey ERP initialize a legal entity's general ledger with a standard Chart of Accounts (SKR03 or SKR04) and post opening balances (Eröffnungsbilanz / Saldenvortrag) in strict compliance with GoBD, HGB § 240 / § 242 / § 252 Abs. 1 Nr. 1 (Bilanzidentität), and DATEV EXTF posting standards?
...
2. **Handelsgesetzbuch (HGB)**:
   - *HGB § 240 Abs. 1–2*: Duty to set up an inventory (Inventar) at business inception and opening of each fiscal year.
   - *HGB § 242 Abs. 1*: Obligation to set up an opening balance sheet (Eröffnungsbilanz).
...
1. **Chart of Accounts Template Structure**:
   - NeoDonkey ERP stores standard chart of accounts templates as structured JSON definitions under `operating-model/information/` (e.g., `_chart-skr03.md` or JSON definition equivalents).
...
3. **Opening Balance (Eröffnungsbilanz / Saldenvortrag) Posting Invariants**:
   - Opening balance entries are posted via a specialized `journal-entry` document type with `entry-type: "opening-balance"`.
   ...
   - **Structural Invariant**: The net balance of Opening Balance Sheet Account `9000` (EBK) MUST equal exactly `0.00 EUR` across all opening balance postings for a financial year. If `sum(Debits) != sum(Credits)`, the opening balance journal entry MUST be rejected by kernel invariant checks.
...
### Issue 2: Post opening balance (Eröffnungsbilanz) carryforward journal entries with strict debit/credit invariant validation against EBK 9000

- **Title**: `feat(ledger): post opening balance (Eröffnungsbilanz) carryforward journal entries with strict debit/credit invariant validation against EBK 9000`
...
- **Priority**: `p1`
- **Status**: `ready`
- **Verification**: A unit test in `test/opening-balance.test.js` creates an opening balance journal entry for SKR03 accounts (`1000 Kasse`, `1200 Bank`, `0800 Gezeichnetes Kapital`), asserting that balanced opening postings debiting `1000`/`1200` and crediting `0800` against EBK `9000` result in EBK `9000` balance equal to `0.00 EUR`. An unbalanced opening balance entry (e.g., total debits exceeding credits by 0.01 EUR) is refused with an explicit `InvariantError("Opening balance sheet account 9000 does not balance to zero")`.
